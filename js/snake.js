...
const keydownListener = (event) => {
	...
	if(
		!paused && length === 1 && (event.keyCode === keys.LEFT 
	...
	if(
		!paused && ((event.keyCode === keys.LEFT && !pressed[keys.RIGHT]) || 
...
function checkOutOfBound(){
	if(
		box.offsetLeft + 10 < 0 || box.offsetTop < 0 
...
function createBox(){
	...
	secondBox.className = "box new";
	thirdBox.className = "box new";
	board.appendChild(newBox);
	board.appendChild(secondBox);
	board.appendChild(thirdBox);
	box_arr.push(newBox);
